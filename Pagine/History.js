      ...
      <SafeAreaView style={ss.safeareaview}>
        <ScrollView>
          <View style={ss.container}>
            <Text  style={ss.h1}>Storico Servizi</Text>
            {
              ritiri.length!=0 ?
                ritiri.map((r, index) => (
                  <Surface key={"indrizzo_"+index} style={[ss.bordogrigio,ss.p10,ss.w100,ss.mt5]}>
                    ...
                    {typeof(r.Note)!=="undefined" && r.Note!==null?<EtichettaSurface etichetta="Dove:">{r.Note.trim()}</EtichettaSurface>:null}
                    {typeof(r.Note2)!=="undefined" && r.Note2!==null?<EtichettaSurface etichetta="Note:">{r.Note2.trim()}</EtichettaSurface>:null}
                    {typeof(r.Dove_2)!=="undefined" && r.Dove_2!==null?<EtichettaSurface etichetta="Secondo indirizzo:">{r.Dove_2.trim()}</EtichettaSurface>:null}
